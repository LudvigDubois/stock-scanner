...
# and exports the results to a TradingView-compatible watchlist file with asset class sections.
# Author: Gemini
# Version: 3.0
...
GAIN_PERCENTILE_1M = 0.75
GAIN_PERCENTILE_3M = 0.75
...
def generate_tradingview_watchlist(results_df):
    ...
    try:
        ...
        # Fetch the exchange and asset_class for all tickers
        all_tickers_info_df = pd.read_sql_query("SELECT ticker, exchange, asset_class FROM tickers_exchange", conn)
        conn.close()
        
        # Merge the scan results with the exchange and asset class info
        merged_df = pd.merge(results_df, all_tickers_info_df, on='ticker')

        # Group the results by their asset class
        grouped = merged_df.groupby('asset_class')
        
        watchlist_parts = []
        # Process each group (e.g., 'Stock', then 'Fund')
        for asset_class, group_df in grouped:
            # Create the delimiter, e.g., "###Stocks"
            # We add an 's' to make it plural
            delimiter = f"###{asset_class}s"
            
            # Format each ticker in the group with its exchange prefix
            formatted_tickers = [
                f"{row['exchange']}:{row['ticker']}"
                for index, row in group_df.iterrows()
            ]
            
            # Combine the delimiter and the tickers for this group
            watchlist_parts.append(f"{delimiter},{','.join(formatted_tickers)}")

        # Join all parts into the final string, separated by commas
...
def run_scan():
    ...
    print("Calculating metrics for all stocks. This may take a moment...")
    ...
    filtered_stocks = latest_data[
        (latest_data['avg_dollar_volume'] >= MIN_DOLLAR_VOLUME) &
        (latest_data['adr_percent'] >= MIN_ADR_PERCENT) &
        (latest_data['close'] >= MIN_CLOSE_PRICE)
    ]
    ...
    print(f"\n1-Month Gainer Threshold (Top {100-GAIN_PERCENTILE_1M*100:.0f}%): {gain_1m_threshold:.2f}%")
    print(f"3-Month Gainer Threshold (Top {100-GAIN_PERCENTILE_3M*100:.0f}%): {gain_3m_threshold:.2f}%")
    ...
    if final_results.empty:
        ...
        output_df = final_results[['ticker', 'close', 'adr_percent', 'avg_dollar_volume', 'gain_1m', 'gain_3m']].copy()
    ...
    # Pass the entire results DataFrame to the watchlist generator
